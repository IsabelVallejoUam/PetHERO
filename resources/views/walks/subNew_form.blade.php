...
<div class="mb-3">
    <label for="route" class="form-label">Paseador</label>
    <select name="walker_id" class="form-control" id="walker_id">
        @foreach ($walkers as $walker)
            <option value="{{$walker->user_id}}">{{$walker->owner->name}}</option>
        @endforeach
    </select>
</div> 
<input type="hidden" name="walk_id" value="null" id="walk_id">